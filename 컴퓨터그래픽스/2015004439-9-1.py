...
def render(a,b,c):
    ...
    Rz = np.array([[np.cos(np.radians(a)), -np.sin(np.radians(a)), 0],
                   ...
                   [0,0,1]])
    Rx =  np.array([[1,0,0],
                   [0, np.cos(np.radians(b)), -np.sin(np.radians(b))],
                   [0, np.sin(np.radians(b)), np.cos(np.radians(b))]])
    Rz1 = np.array([[np.cos(np.radians(c)), -np.sin(np.radians(c)), 0],
...
def main():
    global gVertexArraySeparate, a, b, c

    if not glfw.init():
        return
    window = glfw.create_window(480,480,"2015004439-9-1", None,None)
    if not window:
        glfw.terminate()
        return
